...
- [📝 Markdownlint 構文ルール](adr/markdownlint.md)
...
## Markdownlint 実行コマンド一覧

| コマンド | 実行結果 |
|---------|---------|
| `npm run lint:md` | Markdown の構文・スタイルをチェックする |
| `npm run lint:md:fix` | 自動修正可能なエラーを修正する |
...
コミットがブロックされた場合は、以下のコマンドでエラーを確認し、自動修正を試す。

```bash
npm run lint:md      # Markdown 構文チェック
npm run lint:md:fix  # Markdown 構文の自動修正
npm run lint:text    # 文章内容チェック
npm run lint:text:fix # 文章内容の自動修正
```
